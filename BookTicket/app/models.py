...
class IntermediateAirport(db.Model):
    ...
    def __init__(self, **kwargs):
        ...
        if current_inter_airports >= policy.max_inter_airport:
            raise ValueError(
                f"Không thẻ thêm sân bay trung gian. Tối đa là: {policy.max_inter_airport}."
            )
        ...
        if not (policy.minimum_stop_time <= stop_time <= policy.maximum_stop_time):
            raise ValueError(
                f"Thời gian dừng phải lớn hơn {policy.minimum_stop_time} và nhỏ hơn {policy.maximum_stop_time}. Bạn cung cấp: {stop_time}."
            )
